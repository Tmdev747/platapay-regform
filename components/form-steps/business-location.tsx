...
export function BusinessLocation() {
  ...
  const getLocation = () => {
    if (navigator.geolocation) {
        ...
        (error) => {
          ...
          switch (error.code) {
            case error.PERMISSION_DENIED:
              errorMessage += "Location access was denied. Please use the map to set your location manually."
              break
            case error.POSITION_UNAVAILABLE:
              errorMessage += "Location information is unavailable. Please use the map to set your location manually."
              break
            case error.TIMEOUT:
              errorMessage +=
                "The request to get your location timed out. Please use the map to set your location manually."
              break
            default:
              errorMessage += "An unknown error occurred. Please use the map to set your location manually."
          }

          // Show the map and set default coordinates (Manila)
          setShowMap(true)
          setValue("latitude", "14.5995")
          setValue("longitude", "120.9842")

          // Alert the user with a more helpful message
          alert(errorMessage)
        },
        {
      ...
      alert("Geolocation is not supported by this browser. Please use the map to set your location manually.")
      setShowMap(true)
      // Default to Manila
      setValue("latitude", "14.5995")
      setValue("longitude", "120.9842")
